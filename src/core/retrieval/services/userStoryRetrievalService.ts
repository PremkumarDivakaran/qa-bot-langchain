import { UserStoryVectorStore } from "../../../infrastructure/lib/vectorstore/userStoryVectorStore.js";
import { createChatModel } from "../../../infrastructure/lib/models/index.js";
...
export class UserStoryRetrievalService {
  ...
  async retrieveUserStories(
    ...
    vectorWeight: number = 0.5,  // Decimal value 0-1
    bm25Weight: number = 0.5     // Decimal value 0-1
    ...
    try {
      ...
      if (searchMode === "hybrid") {
        logger.detailed(traceId, `Hybrid search weights - Vector: ${(vectorWeight * 100).toFixed(1)}%, BM25: ${(bm25Weight * 100).toFixed(1)}%`);
      }
  ...
  private async performVectorSearch(
    ...
    try {
      ...
      logger.status(traceId, "🔧 Step 4: Processing search results for ranking", "🔧");
      ...
      if (config.llmReranking.enabled && initialDocuments.length > relevantStoriesLimit) {
        ...
        logger.status(traceId, `Step 4: LLM re-ranking skipped - using ${initialDocuments.length} results as-is`, "⏭️");
        ...
        logger.status(traceId, `Step 4: LLM re-ranking disabled - using ${initialDocuments.length} results as-is`, "⚙️");
        logger.detailed(traceId, "LLM re-ranking is disabled in configuration", "⚙️");
      }
    ...
    try {
      ...
      switch (searchMode) {
        ...
        case "hybrid":
          ...
          logger.detailed(traceId, `Weights - Vector: ${(vectorWeight * 100).toFixed(1)}%, BM25: ${(bm25Weight * 100).toFixed(1)}%`);
          // Weights are already in decimal format (0-1)
          initialDocuments = await this.vectorStore.hybridSearch(query, initialSearchCount, vectorWeight, bm25Weight);
          break;
      ...
      if (config.llmReranking.enabled && documentsWithScores.length > relevantStoriesLimit) {
        ...
        logger.status(traceId, `Step 4: LLM re-ranking skipped - using ${documentsWithScores.length} results as-is`, "⏭️");
        logger.detailed(traceId, `LLM re-ranking skipped: initial results (${documentsWithScores.length}) <= target count (${relevantStoriesLimit})`, "⏭️");
        // No re-ranking, just limit the results
        finalDocuments = documentsWithScores.slice(0, relevantStoriesLimit);
      } else {
        logger.status(traceId, `Step 4: LLM re-ranking disabled - using top ${relevantStoriesLimit} results`, "⚙️");
        logger.detailed(traceId, "LLM re-ranking is disabled in configuration", "⚙️");
        // No re-ranking, just limit the results
        finalDocuments = documentsWithScores.slice(0, relevantStoriesLimit);
      }
...
You are an expert assistant with in-depth knowledge of QA, software testing, and healthcare domain user stories. You are precise, clear, and focused on producing accurate, standardized user stories.`;
  ...
  private calculateInputQualityScore(
    ...
    if (basicChecks.hasDomainRelevance) {
      ...
      // CRITICAL FIX: No relevant stories should lower score, not increase it
      score -= 25; // Penalty for lack of context/relevant examples
      logger.detailed('Domain analysis', 'No relevant user stories found - applying penalty for lack of context');
    }
    ...
    // Final check: Heavily penalize when no relevant user stories are found
    // This ensures the score reflects the system's inability to provide context-aware responses
    if (!vectorDbResults || vectorDbResults.length === 0) {
      score = Math.min(score, 35); // Cap score at 35 when no relevant examples exist
      logger.detailed('Final scoring adjustment', 'No relevant user stories found - capping score at 35 to reflect limited context value');
    }
    ...
    logger.detailed('Quality scoring breakdown', 
      ...
      `relevant stories found: ${vectorDbResults?.length || 0}, ` +
