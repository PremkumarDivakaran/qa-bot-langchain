  ...
  async addUserStories(userStories: Array<{
    ...
    const documents = userStories.map((userStory) => 
      new Document({
        ...
        metadata: {
          storyId: userStory.storyId || `STORY-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
          projectName: userStory.projectName || "Unknown Project",
          title: userStory.title || "Untitled",
          description: userStory.description,
          acceptanceCriteria: userStory.acceptanceCriteria || null,
          priority: userStory.priority || "unknown",
          category: userStory.category || "general",
          risk: userStory.risk || null,
